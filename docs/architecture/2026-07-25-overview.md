...
| `internal/adapters/anthropic` | Outbound: Anthropic's native Messages API (text only in v1; see below). |
...
server tools (web search, phase 3); the compat endpoint silently ignores
`web_search_options`.
...
- **Response:** the upstream call always streams. `parseStream` relays only
  text blocks — thinking and server-tool blocks are dropped and nothing is
  ever emitted as a tool call, since Open WebUI executes tool calls it
  receives. `stop_reason` maps to `finish_reason` (`max_tokens`→`length`,
  `refusal`→`content_filter`, else `stop`). Prompt tokens are input plus
...
- **Errors:** a non-2xx is relayed with its body and status, except that
  401/403 become 502 (llmux's key, not the client's) and Anthropic's 529
  becomes 503; `Retry-After` is passed through. An `error` event before
  `message_start` gets the matching status (overloaded → 503, rate limit →
  429); after it, the SSE stream ends with an error chunk. A connection
  failure is a 502.
...
- **Timeouts and redirects:** no overall client timeout (answers stream as
  long as they take). Dial 10s, response headers 60s, and 90s with no bytes
  at all cancels the stream (Anthropic pings while it works). Redirects are
