...
- **Errors:** a non-2xx is relayed with its status and body; a connection
  failure is a 502; an `error` event mid-stream ends the SSE stream with an
  error chunk.
- **Timeouts:** no overall client timeout (answers stream as long as they
  take; the client's context cancels the call), but response headers must
  arrive within 60s.
