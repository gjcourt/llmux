...
| `internal/ports/outbound` | `ChatProvider` — what a model backend implements; `Metrics` — where telemetry goes. |
| `internal/app` | Routes each request to the first provider whose `Handles(model)` is true, and meters it. |
...
| `internal/adapters/prometheus` | Outbound: `Metrics` as Prometheus series, served on its own listener. |
...
Logging is `slog` text to stderr at debug level. Prometheus metrics —
tokens, outcomes, latency, time to first token, web searches, per provider and
model — are served on a separate listener, `LLMUX_METRICS_ADDR` (default
`:9090`, empty disables); see
[the telemetry design](../design/2026-09-25-telemetry.md).
...
- **Web search:** every request offers the model
  ...
  answer costs ~11–30k input tokens. Merely offering the tool adds ~2.2k
  (Haiku 4.5) – 2.8k (Sonnet 5) input tokens to a request, searched or not;
  without it a short prompt is ~14. So by default
  (`LLMUX_WEB_SEARCH_STREAM_ONLY=true`) only **streamed** requests are
  offered it: Open WebUI's chat replies stream, while its background calls
  (titles, tags, follow-ups, emoji, autocomplete, search queries) are all
  `'stream': False` (checked in v0.11.3 and v0.11.4, `routers/tasks.py`).
  llmux can't tell who is asking, so this applies to **every** non-streaming
  client — including an Open WebUI user who turns off "Stream Chat
  Response", and any other OpenAI-compatible caller — and the answer gives
  no sign search was withheld (a debug log line records it). Set the
  variable to `false` to offer search to every request.
  `max_uses` is probably per request, so a resumed answer could search more
  than N times (unverified). Open WebUI's background calls are
  not offered the tool by default (see below).
...
- **`pause_turn`:** a long server-tool turn can pause. The adapter keeps
  ...
  length`. A failed resume — an HTTP error or an `error` event — keeps the
  upstream status: a JSON client gets it (with `Retry-After` when sent); a
  streaming one, already answered, an error chunk.
...
- **Errors:** a non-2xx is relayed with its body and status, except that
  ...
  event carries the matching status (overloaded → 503, rate limit → 429):
  a non-streaming client gets it; a streaming client that has already been
  sent output gets an error chunk. A connection failure is a 502.
