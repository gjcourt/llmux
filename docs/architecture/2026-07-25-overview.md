...
- **Citations:** each `citations_delta` of type
  ...
  `message.annotations` for JSON), which Open WebUI renders as source chips.
...
- **`pause_turn`:** a long server-tool turn can pause. The adapter keeps
  ...
  usage. If it is still paused after that, the answer ends with
  `finish_reason: length`. A failed resume is mid-answer, so it becomes an
  in-stream error chunk.
